...
class Sensor(threading.Thread):
    ...
    def run(self):
        while self.running.is_set(): # Continue grabbing data from sensor while Flag is set
            time.sleep(0.005)  # Time to sleep in seconds, emulating some sensor process taking time
            self.val = np.random.random((np.random.randint(10,100),3)) # Generate random integers to emulate data from sensor
