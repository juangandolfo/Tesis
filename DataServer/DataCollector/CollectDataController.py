...
from DataServer.Plotter.GenericPlot import *
from DataServer.AeroPy.TrignoBase import *
from DataServer.AeroPy.DataManager import *
import subprocess
import time
import sys
                    ...
                    for channel in range(len(selectedSensor.TrignoChannels)):
                        ...
                        if "EMG" in selectedSensor.TrignoChannels[channel].Name:
                            ...
                            if "TrignoAvanti" in str(selectedSensor) and "RMS 2" in selectedSensor.TrignoChannels[
                                ...
                                plotCount += 1
                                params.Channels_ID.append(selectedSensor.TrignoChannels[channel].Id)
    ...
    def StartVisualization_Callback(self):
        try:
            if self.VisualizationProcess == 0:
                self.VisualizationProcess = subprocess.Popen('bokeh serve --show Visualization/streaming')
            else:
    ...
    def StartCursor_Callback(self):
        try:
            if self.CursorProcess == 0:
                self.CursorProcess = subprocess.Popen(sys.executable + ' Cursor/Cursor.py')
            else:
